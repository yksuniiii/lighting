...
def net(input_feataure):
    conv_num = 4096
    filter_size = 32
    mid1 = tflearn.conv_2d(input_feataure, nb_filter=conv_num,
                                             filter_size=filter_size,
                                             weights_init='xavier',
                                             name='myConv_1')
    mid2 = tf.nn.batch_normalization(mid1, 0, 1.0,
                                                           0.0, 1.0, 1e-5,
                                                           'BatchNorm')
    mid3 = tflearn.activations.leaky_relu(mid2)

    network = tflearn.fully_connected(input_feataure, 4096, activation='tanh')  
    network = tflearn.fully_connected(network, 27, activation='softmax')  
    return network
